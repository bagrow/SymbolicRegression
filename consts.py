...
function_dict = {'Koza-1': (lambda x: x**4 + x**3 + x**2 + x, {'type': 'urand',
                                                               'a': -1,
                                                               'b': 1,
                                                               'size': 20,
                                                               'factor': 1}),

                 'Koza-2': (lambda x: x**5 - 2 * x**3 + x, {'type': 'urand',
                                                            'a': -1,
                                                            'b': 1,
                                                            'size': 20,
                                                            'factor': 2}),

                 'Koza-3': (lambda x: x**6 - 2 * x**4 + x**2, {'type': 'urand',
                                                               'a': -1,
                                                               'b': 1,
                                                               'size': 20,
                                                               'factor': 3}),

                 'Nguyen-1': (lambda x: x**3 + x**2 + x, {'type': 'urand',
                                                          'a': -1,
                                                          'b': 1,
                                                          'size': 20,
                                                          'factor': 4}),

                 'Nguyen-3': (lambda x: x**5 + x**4 + x**3 + x**2 + x, {'type': 'urand',
                                                                        'a': -1,
                                                                        'b': 1,
                                                                        'size': 20,
                                                                        'factor': 5}),

                 'Nguyen-4': (lambda x: x**6 + x**5 + x**4 + x**3 + x**2 + x, {'type': 'urand',
                                                                               'a': -1,
                                                                               'b': 1,
                                                                               'size': 20,
                                                                               'factor': 6}),

                 'Nguyen-5': (lambda x: np.sin(x**2) * np.cos(x) - 1, {'type': 'urand',
                                                                       'a': -1,
                                                                       'b': 1,
                                                                       'size': 20,
                                                                       'factor': 7}),

                 'Nguyen-6': (lambda x: np.sin(x) + np.sin(x + x**2), {'type': 'urand',
                                                                       'a': -1,
                                                                       'b': 1,
                                                                       'size': 20,
                                                                       'factor': 8}),

                 'Nguyen-7': (lambda x: plog(x + 1) + plog(x**2 + 1), {'type': 'urand',
                                                                       'a': 0,
                                                                       'b': 2,
                                                                       'size': 20,
                                                                       'factor': 9}),

                 'Nguyen-8': (lambda x: psqrt(x), {'type': 'urand',
                                                   'a': 0,
                                                   'b': 4,
                                                   'size': 20,
                                                   'factor': 10}),

                 'Keijzer-1': (lambda x: 0.3 * x * np.sin(2 * np.pi * x), {'type': 'uniform',
                                                                           'a': -1,
                                                                           'b': 1,
                                                                           'step': 0.1,
                                                                           'factor': 0}),

                 'Keijzer-2': (lambda x: 0.3 * x * np.sin(2 * np.pi * x), {'type': 'uniform',
                                                                           'a': -2, 'b': 2,
                                                                           'step': 0.1,
                                                                           'factor': 11}),

                 'Keijzer-4': (lambda x: x**3 * np.exp(-x) * np.cos(x) * np.sin(x) * (np.sin(x)**2 * np.cos(x) - 1),
                               {'type': 'uniform',
                                'a': 0,
                                'b': 10,
                                'step': 0.05,
                 ...
                 'scaled-sinc': (lambda x: 5 * pdivide(np.sin(x), x), {'type': 'urand',
                                                                       'a': 0,
                                                                       'b': 10,
                                                                       'size': 30}),

                 'AutomaticFrenchCurve': (lambda x: 4.26 * (np.exp(-x) - 4 * np.exp(-2 * x) + 3 * np.exp(-3 * x)),
                                          {'type': 'urand',
                                           'a': 0,
                                           'b': 3.25,
                                           'size': 30}),

                 'ChebyshevPolynomial': (lambda x: 3 * np.cos(3 * np.arccos(x)), {'type': 'urand',
                                                                                  'a': -1,
                                                                                  'b': 1,
                                                                                  'size': 30}),

                 'Ripple': (lambda x: (x[0] - 3.) * (x[1] - 3.) + 2. * np.sin(x[0] - 4.) * (x[1] - 4.),
                            {'type': 'urand',
                             'a': [0, 0],
                             'b': [5, 5],
                             'size': 30}),

                 'RatPol3D': (lambda x: 30. * (x[0] - 1.) * (x[2] - 1.) / x[1]**2 / (x[0] - 10.),
                              {'type': 'urand',
                               'a': [0.05, 1, 0.05],
                               'b': [5, 2, 5],
                               'size': 30}),   # per dimension

                 'UBall5D': (lambda x: 10. / (5. + (x[0]-3.)**2 + (x[1]-3.)**2 + (x[2]-3.)**2 + (x[3]-3.)**2 + (x[4]-3.)**2),
                             {'type': 'urand',
                              'a': [0.05]*5,
                              'b': [5.]*5,
                              'size': 30}),

                 'identity': (lambda x: x, {'type': 'urand',
                                            'a': -1,
                                            'b': 1,
                                            'size': 30})
                 }
